...
- Public API added to `rich_markdown.py` (all re-exported from `messages/`): `scan_media`, `MediaScan` (`markdown`, `files` — a `remote` list was dropped in review, nothing consumed it), `RichFile` (`id`, `path`, `caption`, `kind`), `MediaResolutionError` / `AmbiguousMediaError` (both `ValueError`, so every surface's existing 400 / exit-2 path already covers them), `media_kind`, `rich_file_reference`, `make_rich_file_id`, `RICH_FILE_SCHEMES`, `RICH_FILE_ID_RE`, `MAX_RICH_FILE_ID_CHARS`, `PHOTO_/VIDEO_/AUDIO_SUFFIXES`.
