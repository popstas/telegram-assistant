...
- доступ запрещён политикой (`access denied`): сконфигурированная политика
  `telegram.access` не даёт требуемого уровня (`read`/`write`) для чата или
  папки назначения. Отказ «громкий»: HTTP `403`, ненулевой код выхода CLI
  (`3`) и структурная запись в лог (`chat ref`, требуемый уровень, сработавшее
  правило или его отсутствие). При `access is None` действует allow-all, и эта
  ошибка не возникает;
- ссылка на сущность (entity) не разрешается: не найдена
  (`EntityNotFoundError`) или неоднозначна (`AmbiguousEntityError`).
...
- серия отказов доступа (`access_denied`) подряд — возможно, неверно
  сконфигурирована политика `telegram.access`.

Решения о доступе логируются структурно: при отказе пишется запись
`access_denied` с `chat ref`, требуемым уровнем и сработавшим правилом (или
его отсутствием).
