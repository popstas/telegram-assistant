...
def test_folders_remove_chat_dry_run_envelope_present(
    minimal_config_yaml: str,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_folders_remove_chat_dry_run_absent_warns(
    minimal_config_yaml: str,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert payload["warnings"]
    assert any("not in folder" in w for w in payload["warnings"])
    assert backend.removed == []
