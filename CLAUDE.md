...
`messages/` is split more narrowly: `attachments.py` holds surface-level attachment validation, `service.py` handles send/recent/mass send, `reactions.py`, `forwarding.py`, `editing.py`, `pinning.py`, `media_download.py` (download media *from* an existing message — distinct from `downloads.py`, which fetches a URL to a temp file *for* sending), and `search.py` hold their small domain operations, and `messages/telethon_backend.py` contains the send/read/reaction/forward/edit/pin/download/search Telethon adapters.
