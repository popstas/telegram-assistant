...
`data/config.yml` is loaded by `config.loader.load_config()`. Schema (Pydantic) is in `config/models.py`. Notable: `telegram.default_chat_folder.folder_name` is the default for CLI `--folder-name` and for placing newly created groups; chat folders are never auto-created — if the configured folder is missing, the service returns an error. `telegram.download_root` (optional, default: the system temp dir) confines where **remote** (HTTP/MCP) `messages download` may write: a caller-supplied `out_dir` is resolved against this root (relative values join into it) and rejected with 400 when it escapes, so a READ-only remote identity cannot pick an arbitrary write location (`_resolve_download_dir` in `http_api/messages.py`, shared by the HTTP router and the MCP tool). The CLI (local, trusted) passes `--out`/`--dir` straight through and is not confined. `telegram.pin_min_interval_seconds` (default **2.0**; `0` disables) is the shared pin/unpin pacing interval described under Architecture — it is read per request, so raising it from `0` at runtime takes effect on the next call (the HTTP `on_swap` builds the gate store when that happens, mirroring the folder cache).
...
`folder_cache_ttl` (seconds, default **300**; `0` disables) tunes the persistent folder-membership cache: when any `folder:`/`folder_id:` rule is present, the chat→folders map is built once from `InputPeer` ids (`list_folder_chat_ids()` — **no per-chat `get_entity`**, the old perf sink) and persisted to SQLite (`folder_membership_cache` table via `FolderMembershipCache`), so a fresh CLI process reuses it; a fetch failure serves the stale cached map (bounded by TTL + outage), and the cache is cleared in the hot-reload `on_swap` so access-rule edits apply cleanly. Config changes are hot-reloaded: a `watchdog` observer on `data/config.yml` re-runs `load_config()` with a 2s debounce and atomically swaps `app.state.config` on success (keeping the last-good config on parse/validation error), so access-rule edits apply live without a restart.
