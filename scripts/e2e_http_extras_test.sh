...
# to run *after* scripts/e2e_test.sh so "${GROUP_TITLE}" already exists in the
# "${FOLDER}" folder.
#
# Targets:
#   * Every message-shaped step goes to Saved Messages (${TEST_CHAT}).
#   * Only genuinely group-shaped steps (topics, members, folder membership
#     round-trip, group rename) touch "${GROUP_TITLE}".
#
# Prerequisites:
...
# Resolve Saved Messages — every message-shaped step below addresses it by id.
...
# Resolve the chat id for the test group via the folder endpoint.
step "resolve chat id for '${GROUP_TITLE}' via /telegram/folders/${FOLDER}"
chat_id=$(curl -sS "${auth_header[@]}" "${BASE_URL}/telegram/folders/${folder_enc}" \
    | jq -r --arg t "${GROUP_TITLE}" '.chats[] | select(.title==$t) | .chat_id')
if [[ -z "${chat_id}" ]]; then
    echo "could not find '${GROUP_TITLE}' in folder '${FOLDER}' — run scripts/e2e_test.sh first" >&2
    exit 1
fi
...
addchat_resp=$(curl -sS -X POST "${auth_header[@]}" "${json_header[@]}" \
    -d "${addchat_payload}" \
    "${BASE_URL}/telegram/folders/${folder_enc}/chats" || true)
echo "${addchat_resp}" | jq . 2>/dev/null || echo "${addchat_resp}"

# --- media / scheduled send, reactions, forward, mute ----------------------
# These target Saved Messages. The scheduled send is deferred far enough to be
# cancellable by hand; the mute is rounded back with an unmute.

step "POST /telegram/messages (media) — attach a remote URL"
media_payload=$(jq -nc --arg cid "${test_chat_id}" \
...
step "POST /telegram/messages (scheduled) — defer by 600s into ${TEST_CHAT}"
sched_payload=$(jq -nc --arg cid "${test_chat_id}" \
...
if [[ -n "${message_id}" && "${message_id}" != "null" ]]; then
    ...
    react_payload=$(jq -nc --arg cid "${test_chat_id}" --argjson mid "${message_id}" \
    ...
    clear_payload=$(jq -nc --arg cid "${test_chat_id}" --argjson mid "${message_id}" \
        '{telegram_chat_id: ($cid|tonumber), message_id: $mid, clear: true}')
    curl -sS -X POST "${auth_header[@]}" "${json_header[@]}" \
        -d "${clear_payload}" \
        "${BASE_URL}/telegram/messages/reactions" | jq '{telegram_message_id, emoji, cleared}'

    step "POST /telegram/messages/forward (message ${message_id} into the same chat)"
    fwd_payload=$(jq -nc --arg cid "${test_chat_id}" --argjson mid "${message_id}" \
...
# All target Saved Messages. The delete target is sent by this same server
...
b64_payload=$(jq -nc --arg cid "${test_chat_id}" --arg b64 "${b64}" \
...
fileurl_payload=$(jq -nc --arg cid "${test_chat_id}" \
...
dry_delete_payload=$(jq -nc --arg cid "${test_chat_id}" \
...
step "POST /telegram/notifications/mute then /unmute (round-trip for chat ${test_chat_id})"
mute_payload=$(jq -nc --arg cid "${test_chat_id}" \
    '{telegram_chat_id: ($cid|tonumber), duration_hours: 1}')
curl -sS -X POST "${auth_header[@]}" "${json_header[@]}" \
    -d "${mute_payload}" \
    "${BASE_URL}/telegram/notifications/mute" | jq '{telegram_chat_id, muted}'
unmute_payload=$(jq -nc --arg cid "${test_chat_id}" '{telegram_chat_id: ($cid|tonumber)}')
curl -sS -X POST "${auth_header[@]}" "${json_header[@]}" \
    -d "${unmute_payload}" \
...
curl -sS -X DELETE "${auth_header[@]}" "${json_header[@]}" \
    ...
    "${BASE_URL}/telegram/folders/${folder_enc}/chats" | jq '{folder_id, already_absent}'
curl -sS -X POST "${auth_header[@]}" "${json_header[@]}" \
    -d "${remove_chat_payload}" \
    "${BASE_URL}/telegram/folders/${folder_enc}/chats" | jq '{folder_id, already_in_folder}'

# --- group rename round-trip (rename then rename back) ----------------------
# Rename the test group -> "<title> (renamed)" by id, then rename back so every
# later --chat-name / title lookup (and other scripts) still resolve.
HTTP_CHAT_RENAMED="${GROUP_TITLE} (renamed)"
...
step "POST /telegram/groups/rename — rename back to '${GROUP_TITLE}'"
grename_back_payload=$(jq -nc --arg cid "${chat_id}" --arg title "${GROUP_TITLE}" \
